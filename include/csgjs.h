...
namespace csgjscpp
{
    ...
    bool modeltoply(const std::string filename, const Model &model)
    {
    ...
    */
    CSGJSCPP_VECTOR<Polygon> csgfixtjunc(const CSGJSCPP_VECTOR<Polygon> &originalpolygons)
    {
        ...
        for (const auto &originalpoly : originalpolygons)
        {
            ...
            {
                auto pos = CSGJSCPP_FIND_IF(uvertices.begin(), uvertices.end(), [v](const IndexedVertex &a)
                                            { return a.vertex == v; });
                            ...
                            for (const auto &matchingsidetag : matchingsides)
                            {
                                ...
                                else
                                {
                                    ...
                                    if ((t > 0.0f) && (t < 1.0f))
                                    {
                                        ...
                                        { // TODO - shouldn't this be epsilon constant?
